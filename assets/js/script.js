...
function checkEmptyInput() {
  ...
  const familyMember = document.querySelector('.family-member');
  if(name === "" || address === "" || mobileNumber === "" || emailAddress === "" || familyMember === "") {
    return false
  } else {
    return true
  }
...
form.addEventListener('submit', (e) => {
  ...
  // if(checkEmptyInput()) {    
    // console.log(checkEmptyInput())
    fetch('http:localhost:3333/emaildata', {
      method: "post",
      // mode: 'cors',
      // credentials: 'include',
      headers: {
...
const locations = (new Date()).toString().split('(')[1].split(" ")[0];
    const numberInput = document.querySelector('.dial-code');
    dialCode.forEach(data => {
      if(locations.includes(data.name)) {
        numberInput.innerHTML = data.dial_code;
        code = data.dial_code;
      }
    })
}
